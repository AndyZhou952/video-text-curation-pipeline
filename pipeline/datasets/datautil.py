...
def main(args):
    ...
    if args.ocr_box_max is not None:
        assert "num_boxes" in data.columns
        data = data[data["num_boxes"] <= args.ocr_box_max]
    if args.ocr_single_max is not None:
        assert "max_single_percentage" in data.columns
        data = data[data["max_single_percentage"] <= args.ocr_single_max]
    if args.ocr_total_max is not None:
        assert "total_text_percentage" in data.columns
        data = data[data["total_text_percentage"] <= args.ocr_total_max]
    ...
    if args.vid_only:
        data = data[~data["path"].str.lower().str.endswith(IMG_EXTENSIONS)]
    # filter by minimum seconds
    if args.secondsmin:
        assert "num_frames" in data.columns and "fps" in data.columns
        data = data[data["num_frames"] / data["fps"] >= args.secondsmin]
    # filter by maximum seconds:
    if args.secondsmax:
        assert "num_frames" in data.columns and "fps" in data.columns
        data = data[data["num_frames"] / data["fps"] <= args.secondsmax]
...
def get_output_path(args, input_name):
    ...
    if args.filesize:
        name += "_filesize"
    if args.fsmax is not None:
        name += f"_fsmax{int(args.fsmax)}"
    if args.fmin is not None:
        name += f"_fmin{int(args.fmin)}"
    if args.fmax is not None:
        name += f"_fmax{int(args.fmax)}"
    if args.fpsmax is not None:
        name += f"_fpsmax{int(args.fpsmax)}"
    if args.hwmax is not None:
    ...
    if args.ocr_total_max is not None:
        name += f"_ocrtotalmax{args.ocr_total_max:.1f}"
    if args.aesmin is not None:
    ...
    if args.vid_only:
        name += "_vid"
    if args.secondsmax:
        name += f"_secondsmax{args.secondsmax:.1f}"
    if args.secondsmin:
        name += f"_secondsmin{args.secondsmin:.1f}"
