...
class Hashing:
    ...
    def encode_image(self, image_array: Optional[np.ndarray] = None) -> str:
        """
        Generate hash for a single image from a numpy array.
    ...
    def encode_images(self, paths=None, image_arrays=None, num_enc_workers: int = cpu_count()):
        """
        Generate hashes for a batch of images given their paths and numpy arrays.

        Args:
            paths: List of image paths (used as keys for the output dictionary).
            image_arrays: List of numpy arrays representing images (used for hash computation).
        ...
        Returns:
            dictionary: A dictionary that contains a mapping of paths and corresponding 64 character hash string
                        such as {'path1': 'hash_string1', 'path2': 'hash_string2', ...}

        Example:
        ```
        encodings = myencoder.encode_images(paths=['path1', 'path2'], image_arrays=[array1, array2])
        ```
        """
